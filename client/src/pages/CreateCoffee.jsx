...
const CreateCoffee = ({ coffeeOptions }) => {
    ...
    const allowsRoast = useMemo(() => {
        if (!selectedCaffeine) return false;
        const name = (selectedCaffeine.name || '').toLowerCase();
        // Brewed coffee styles typically include the word "brew"
    ...
    const createCoffee = async (event) => {
        ...
        try {
            ...
            const response = await UserRecipesAPI.createUserRecipe(payload);
            if (response && response.id) {
                // Redirect to the detail page of the newly created recipe
                navigate(`/coffees/${response.id}`);
            }
        } catch (error) {
            console.error("Coffee creation failed", error);
        }
    };
    ...
    return (
        <div className={`create-coffee-container ${coffeeRecipe.is_iced ? 'iced-theme' : 'hot-theme'}`}>
            ...
            <form onSubmit={createCoffee} className="create-coffee-form">
                <label>
                    Recipe Name
                    <input
                        type="text"
                ...
                <label className='is-iced-container'>
                    Is Iced
                    <input
                        type="checkbox"
                        name="is_iced"
                ...
                <button type="submit" className="submit-button">Create Coffee Recipe</button>
            </form>
        </div>
    )
}
